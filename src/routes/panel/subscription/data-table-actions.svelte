	...
	const destroyForm = superForm(formDestroy, {
		...
		onResult: ({ result }) => {
			if (result.type === 'success') {
				...
				toast.error(result.error?.message || 'Failed to delete subscription');
			}
		}
	});
...
<DropdownMenu.Root>
	...
	<DropdownMenu.Content>
		...
		<DropdownMenu.Item>
			<Button size="sm" variant="ghost"><UserMinus /> Unsubscribe</Button>
		</DropdownMenu.Item>
...
<Dialog.Root bind:open={isOpenDestroy}>
	<Dialog.Content>
		<Dialog.Header>
			<Dialog.Title  class="text-primary font-semibold">Delete Subscriber</Dialog.Title>
			<Dialog.Description>
